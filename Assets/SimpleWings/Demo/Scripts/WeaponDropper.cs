...
public class WeaponDropper : MonoBehaviour
{
	...
	private void SpawnWeapon()
	{
		...
		Collider[] colliders = gameObject.transform.parent.transform.parent.GetComponentsInChildren<Collider>();
		// Debug.Log(gameObject.transform.parent.transform.parent);
		for (int i=0; i<colliders.Length; i++) {
			// Debug.Log(colliders[i]);
			Collider[] weps = wep.GetComponentsInChildren<Collider>();
			for (int j=0; j<weps.Length; j++) {
				Physics.IgnoreCollision(weps[j], colliders[i]);
			}
